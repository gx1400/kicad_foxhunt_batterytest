...
6. **`IO39` was also incorrectly listed as spare**, same root cause as item 5 — live schematic trace shows it's wired to the PCA9555's own `INT` output (`PERIPH_IO_INT`), not freed for a heartbeat LED (see the pin table, pin 32). **IO3 (pin 15)** was the last genuinely spare pin; it's now spent on the audio circuit's XSMT mute control (see the pin table above) — nothing left spare.

36 of 36 available GPIOs now assigned a purpose — **none genuinely spare anymore**; IO3 (pin 15), the last one, is now spent on the I2S DAC's XSMT mute control. Any future need for a GPIO will have to reclaim one of the existing assignments. Cross-checking the live schematic (not just this doc) shows most are actually wired today — see the **Wired** column in the table above for the pin-by-pin breakdown; a few (SA818S UART/PTT, forward-power ADC/comparator) are still planning-only reservations.
